...
s_path='C:\Users\evanh\Box\Cornell\Fall_2021\Carp_UAV\MicaSense\0005SET\';
...
v = VideoWriter('C:\Users\evanh\Box\Cornell\Fall_2021\Carp_UAV\Velocimetry\carp_005.avi');
open(v);

%Loop over images and display them
therm_imgs = dir(fullfile(s_path, '**\*6.tif'));
n_img = str2num(sprintf('%03d', length(therm_imgs)));
for i=030:550 %Must be 3 digits, beginning w/ 001 
    ...
    imagesc(I1,[7150 7220]) %Set thermal resolution scale here
    axis image
    colormap(jet(256))
    colorbar
    title (num2str(fname))
