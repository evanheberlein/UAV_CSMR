...
s_path='C:\Users\evanh\Box\Cornell\Fall_2021\Carp_UAV\MicaSense\0006SET\';
...
v = VideoWriter('C:\Users\evanh\Box\Cornell\Fall_2021\Carp_UAV\UAV_CSMR\Velocimetry\carp_006.avi');
open(v);

%Loop over images and display them
therm_imgs = dir(fullfile(s_path, '**\*6.tif'));
n_img = str2num(sprintf('%03d', length(therm_imgs)));
for i=030:540 %Must be 3 digits, beginning w/ 001 
    ...
    imagesc(I1,[7170 7240]) %Set thermal resolution scale here
    axis image
    colormap(jet(256))
    colorbar
    title (num2str(fname))
